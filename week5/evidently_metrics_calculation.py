...
def calculate_metrics_postgresql(curr, i):
	...
	prediction_drift = result['metrics'][2]['result']['drift_score']
	# modelquality = result['metrics'][3]['result']['current']['mean_abs_error']
	quatile_valued_colum = result['metrics'][3]['result']['current']['value']

	curr.execute(
		"insert into evidently_metrics(timestamp, prediction_drift, num_drifted_columns, share_missing_values, quatile_valued_colum) values (%s, %s, %s,%s, %s)",
		(begin + datetime.timedelta(i), prediction_drift, num_drifted_columns, share_missing_values, quatile_valued_colum)
	)
